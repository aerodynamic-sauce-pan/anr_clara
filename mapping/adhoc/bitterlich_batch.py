...
from mapping.adhoc.geometry import pol2cart
from utils.tools import get_files_path
...
PARSER.add_argument('-s', '--source',
                    ...
                    type=str,
                    default=['./CAPTURES'],
                    help='Source folder containing rgb, ss and depth views.')
...
def export_prediction(export_data):
    csv = pd.DataFrame(columns=['TimeStamp', 'Tree_angle', 'Tree_distance', 'Tree_X', 'Tree_Y', 'Tree_girth'])
    for tree_map, timestamp in tqdm(export_data):
        ...
        for angle, dist in zip(angles, distances):
            cart = pol2cart(angle, dist)
            posXY[0].append(cart[0])
            posXY[1].append(cart[1])
...
def main():
    ...
    for fp_rgb, fp_ss, fp_dep in zip(tqdm(list(fps['rgb'])), list(fps['ss']), list(fps['dep'])):
        timestamp, ext = os.path.splitext(fp_dep)
        timestamp = timestamp.split('/')[-1]
        if ext in ['.png', 'jpg', 'jpeg', 'bmp']:
            imgDepth = cv.cvtColor(cv.imread(fp_dep), cv.COLOR_BGR2GRAY)
        elif ext == '.dep':
            imgDepth = np.loadtxt(fp_dep)
        else:
            raise TypeError('Depth file format not handled. Please use one of the'
                            'following: dep, png, jpg, jpeg, bmp.')
        imgSS = cv.cvtColor(cv.imread(fp_ss), cv.COLOR_BGR2GRAY)
        imgRGB = cv.cvtColor(cv.imread(fp_rgb), cv.COLOR_BGR2RGB)
        ...
        export_data.append((tree_map, timestamp))
