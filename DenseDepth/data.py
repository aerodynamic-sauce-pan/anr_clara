...
def get_nyu_train_test_data(batch_size):
    ...
    train_generator = NYU_BasicAugmentRGBSequence(batch_size=batch_size, shape_rgb=shape_rgb, shape_depth=shape_depth)
    test_generator = NYU_BasicRGBSequence(batch_size=batch_size, shape_rgb=shape_rgb, shape_depth=shape_depth)
...
class NYU_BasicAugmentRGBSequence(Sequence):
    ...
    def __init__(self, batch_size, shape_rgb, shape_depth, is_flip=False, is_addnoise=False, is_erase=False):
        #self.data = data
        ...
        for img_name in os.listdir(self.rgbdir):
            if os.path.splitext(img_name)[1] == '.png':
                if os.path.isfile(os.path.join(self.depthdir, img_name)):
                    self.eptlist.append(img_name)
        self.lens=len(self.eptlist)
        #from sklearn.utils import shuffle
        #self.dataset = shuffle(self.dataset, random_state=0)

        #self.N = len(self.dataset)
        import random
        random.shuffle(self.eptlist)

    def __len__(self):
        return int(np.ceil(self.lens / float(self.batch_size)))
    '''
    ...
    def __getitem__(self, idx, is_apply_policy=True):
        ...
        # Augmentation of RGB images
        for i in range(batch_x.shape[0]):
            index = min((idx * self.batch_size) + i, self.lens - 1)

            filename = self.eptlist[index]

            x = np.clip(np.asarray(Image.open( self.rgbdir+filename ).convert("RGB")).reshape(480,640,3)/255,0,1)
            y = np.clip(np.asarray(Image.open( self.depthdir+filename ).convert("F")).reshape(480,640,1),0,self.maxDepth)
            #y = DepthNorm(y, maxDepth=self.maxDepth)
...
class NYU_BasicRGBSequence(Sequence):
    ...
    def __init__(self, batch_size,shape_rgb, shape_depth):
        #self.data = data
        #self.dataset = dataset
        self.batch_size = batch_size
        #self.N = len(self.dataset)
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):            
            ...
            x = np.clip(np.asarray(Image.open( self.rgbdir+filename).convert("RGB")).reshape(480,640,3)/255,0,1)
            y = np.asarray(Image.open(self.depthdir+filename).convert("F"), dtype=np.float32).reshape(480,640,1).copy().astype(float) / 1
            #y = DepthNorm(y, maxDepth=self.maxDepth)
