...
class Network(object):
    ...
    def conv2d(self, inputs, kernel_size, stride, out_channels, name=None,
               padding='SAME'):
    ...
    def split_conv2d(self, inputs, kernel_size, rate, out_channels, name=None,
                     padding='SAME', both_atrous=False):
        in_channels = inputs.get_shape().as_list()[-1]
        weight_shape = [kernel_size, kernel_size, in_channels, out_channels]
        initializer = tf.contrib.layers.xavier_initializer()
        if self.initializer is 'he':
        ...
        if both_atrous:
            ...
            outA = tf.nn.conv2d(inputs, kernelA, strides=strides,
                                padding=padding)
            outB = tf.nn.atrous_conv2d(inputs, kernelB, rate, padding=padding)
        ...
        if self.training:
            ...
            with tf.control_dependencies([train_mean, train_var]):
                return tf.nn.batch_normalization(inputs, batch_mean, batch_var,
                                                 scale=gamma, offset=beta,
                                                 variance_epsilon=0.00001)
        else:
            return tf.nn.batch_normalization(inputs, moving_mean, moving_var,
                                             scale=gamma, offset=beta,
                                             variance_epsilon=0.00001)

    def pool(self, x, k_size, strides, name=None, padding='SAME'):
        return tf.nn.max_pool(x, [1, k_size, k_size, 1],
                              strides=[1, strides, strides, 1],
                              padding=padding)

    def atrous(self, inputs, kernel_size, rate, out_channels, name=None,
               padding='SAME'):
        in_channels = inputs.get_shape().as_list()[-1]
        weight_shape = [kernel_size, kernel_size, in_channels, out_channels]
        initializer = tf.contrib.layers.xavier_initializer()
    ...
    def tconv2d(self, inputs, kernel_size, out_channels, stride, name=None,
                padding='SAME'):
        in_channels = inputs.get_shape().as_list()[-1]
        ...
        return tf.nn.conv2d_transpose(inputs, kernel, new_shape,
                                      strides=[1, stride, stride, 1],
                                      padding=padding)
