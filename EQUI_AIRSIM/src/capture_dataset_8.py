...
class cube2sph_2():
        ...
        if mode in [0, 2, 5]:
            for cam_elt in self.camera_list:
                self.responses_list.append(
                    airsim.ImageRequest(str(cam_elt), mode, False, False))
        else:
    ...
    def process_img_request(self):
        ...
        for i, response in enumerate(responses):
            if response.pixels_as_float:
                ...
                depth = np.array(airsim.get_pfm_array(response),
                                 dtype=np.float32)
                self.cube_depth[i, :] = depth.reshape(response.height,
                                                      response.width)
            else:
                if VERBOSE:
                    print("Type %d, size %d" % (response.image_type,
                                                len(response.image_data_uint8)))
                rgb = np.frombuffer(response.image_data_uint8,
                                    dtype=np.uint8).reshape(response.height,
                                                            response.width, 3)
    ...
    def create_equi_rgb(self, save_dir):
        airsim.write_png(os.path.normpath(os.path.join(save_dir, str(self.ti)+'_'+str(self.view)+".png")),
                         self.output)
        if VERBOSE:
            print("Done for "+str(self.ti)+'_'+str(self.view)+".png")

    # def create_cub_rgb(self, idx, save_dir):
    #     airsim.write_png(os.path.normpath(os.path.join(save_dir, str(self.ti)+'_'+str(self.camera_list[idx])+'_'+str(self.view)+".png")),
    #                      self.cube_rgb[idx, :])
    #     if VERBOSE:
